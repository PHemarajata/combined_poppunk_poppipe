...
ulimit -v 8388608  # Virtual memory limit in KB
ulimit -m 8388608  # Physical memory limit in KB
ulimit -s 8192       # 8MB stack limit
ulimit -c 0          # Disable core dumps

echo "Memory limits set: Virtual/Physical=8 GB"
echo "Using 2 threads for PopPUNK."

# Create database with a fallback for reduced threads
if ! poppunk --create-db --r-files staged_files.list \
    --output poppunk_db --threads 2; then
...
if poppunk --fit-model bgmm --ref-db poppunk_db --output poppunk_fit --threads 2 --reciprocal-only --count-unique-distances --max-search-depth 20 --K 3 --no-plot; then
