...
# It maps the sample names to the staged FASTA files.
...
# Run PopPUNK assignment with fallbacks for stability
if poppunk_assign --db "poppunk_db" --query staged_all_files.list --output poppunk_full --threads 8 --no-plot ; then
    echo "✅ PopPUNK assignment completed successfully."
elif poppunk_assign --db "poppunk_db" --query staged_all_files.list --output poppunk_full --threads 2 --no-plot ; then
    echo "✅ PopPUNK assignment completed with reduced threads."
elif poppunk_assign --db "poppunk_db" --query staged_all_files.list --output poppunk_full --threads 1 --no-plot ; then
