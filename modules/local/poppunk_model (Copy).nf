process POPPUNK_MODEL {
    ...
    echo "Using ${task.cpus} threads for PopPUNK database creation"
    
    # Create database with conservative settings
    if poppunk --create-db --r-files staged_files.list \\
        --output poppunk_db --threads ${task.cpus}; then
        echo "✅ PopPUNK database created successfully"
    else
        echo "⚠️  Database creation failed, trying with reduced threads..."
    ...
    echo "Using ${task.cpus} threads for model fitting with segfault prevention"
    
    # Try model fitting with conservative settings first
    if poppunk --fit-model bgmm --ref-db poppunk_db \\
        ...
        echo "⚠️  Model fitting failed, trying with reduced complexity..."
        
        # Fallback 1: Reduce threads and simplify parameters
        if poppunk --fit-model bgmm --ref-db poppunk_db \\
            --output poppunk_fit --threads 2 \\
            --max-search-depth 10 \\
            --K 2 \\
            --no-plot; then
            ...
            echo "⚠️  Second attempt failed, trying minimal settings..."
            
            # Fallback 2: Minimal settings
